...
#
# Modifed: Adam Asay, INVITE Networks
# Data: 2015
# This script will call another script if need to configure DMVPN 
#
# **** End License ****
#
        ...
        foreach my $esp_group (@esp_groups) {
            ...
            if (@esp_group_proposals == 0) {
                vpn_die(["vpn", "ipsec", "esp-group", $esp_group, "proposal"],
                    "$vpn_cfg_err No proposals configured for ESP group \"$esp_group\"" . ".  At least one proposal required.\n");
            } elsif (@esp_group_proposals > VPN_MAX_PROPOSALS) {
                vpn_die(
                    ["vpn", "ipsec", "esp-group", $esp_group],
                ...
                foreach my $esp_group_proposal (@esp_group_proposals) {
                    ...
                    {
                        vpn_die(
                            ["vpn", "ipsec", "esp-group", $esp_group, "proposal", $esp_group_proposal, "encryption"],
                            "$vpn_cfg_err No encryption specified for ESP group " . "\"$esp_group\" proposal $esp_group_proposal.\n"
                        );
                    ...
                    {
                        vpn_die(
                            ["vpn", "ipsec", "esp-group", $esp_group, "proposal", $esp_group_proposal, "hash"],
                            "$vpn_cfg_err No hash specified for ESP group \"$esp_group\"" . " proposal $esp_group_proposal.\n"
                        );
                    }
                }
            }
        ...
        foreach my $ike_group (@ike_groups) {
            ...
            if (@ike_group_proposals == 0) {
                vpn_die(["vpn", "ipsec", "ike-group", $ike_group, "proposal"],
                    "$vpn_cfg_err No proposals configured for IKE group \"$ike_group\"" . ".  At least one proposal required.\n");
            } elsif (@ike_group_proposals > VPN_MAX_PROPOSALS) {
                vpn_die(
                    ["vpn", "ipsec", "ike-group", $ike_group],
                    "$vpn_cfg_err A total of "
                        . @ike_group_proposals
                ...
                foreach my $ike_group_proposal (@ike_group_proposals) {
                        ...
                        || $ike_group_proposal_encryption eq "")
                    {
                        vpn_die(["vpn", "ike-group", $ike_group, "proposal", $ike_group_proposal, "encryption"],
                            "$vpn_cfg_err No encryption specified for IKE group " . "\"$ike_group\" proposal $ike_group_proposal.\n");
                    }
                        ...
                        || $ike_group_proposal_hash eq "")
                    {
                        vpn_die(["vpn", "ike-group", $ike_group, "proposal", $ike_group_proposal, "hash"],
                            "$vpn_cfg_err No hash specified for IKE group \"$ike_group\"" . " proposal $ike_group_proposal.\n");
                    }
                }
            }
        }
    ...
    if ($local_key_file ne $running_local_key_file) {
        ...
        if ($local_key_file !~ /^\//) {
            vpn_die(["vpn", "rsa-keys", "local-key", "file"],
                "$vpn_cfg_err Invalid local RSA key file path \"$local_key_file\"." . "  Does not start with a '/'.\n");
        }
        if ($local_key_file =~ /[^a-zA-Z0-9\.\-\_\/]/g) {
            vpn_die(
                ["vpn", "rsa-keys", "local-key", "file"],
        ...
        if ($local_key_file =~ /\/\//g) {
            vpn_die(["vpn", "rsa-keys", "local-key", "file"],
                "$vpn_cfg_err Invalid local RSA key file path \"$local_key_file\"." . " Contains string \"//\".\n");
        }
        if (-d $local_key_file) {
            vpn_die(["vpn", "rsa-keys", "local-key", "file"],
                "$vpn_cfg_err Invalid local RSA key file path \"$local_key_file\"." . " Path is a directory rather than a file.\n");
        }
    ...
    if (defined($nat_traversal)) {
        if ($nat_traversal eq 'enable') {
            ...
            vpn_die(["vpn", "ipsec", "nat-traversal"],
                "$vpn_cfg_err Invalid value \"$nat_traversal\" specified for " . "'nat-traversal'.  Only \"enable\" or \"disable\" accepted.\n");
        }
    }
    ...
    foreach my $peer (@peers) {
        ...
        if (!defined($peer_ike_group) || $peer_ike_group eq '') {
            ...
            vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "ike-group"],
                "$vpn_cfg_err The IKE group \"$peer_ike_group\" specified for peer " . "\"$peer\" has not been configured.\n");
        }

        my $lip        = $vcVPN->returnValue("ipsec site-to-site peer $peer local-address");
        my $peer_is_v6 = (is_ip_v4_or_v6($peer) && is_ip_v4_or_v6($peer) == 6);
        my $conn_is_v6 = (is_ip_v4_or_v6($lip) && is_ip_v4_or_v6($lip) == 6);

        # Check peer and local-ip consistency
        if ($peer_is_v6) {
            ...
            if (not $conn_is_v6) {
                ...
                vpn_die(
                    ["vpn", "ipsec", "site-to-site", "peer", $peer, "local-address"],
                    "$vpn_cfg_err local-address 'any' is not supported for " . 'IPv6 peers'
                ) if ($lip eq 'any');

                # Protocols don't match
                vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "local-address"], "$vpn_cfg_err local-address and peer protocols " . 'do not match');
            }
        }
        ...
        if (!$vcVPN->exists("ipsec esp-group $def_esp_group")) {
            vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "default-esp-group"],
                "$vpn_cfg_err The ESP group \"$def_esp_group\" specified " . "for peer \"$peer\" has not been configured.\n");
        }
        ...
        if (@tunnels == 0) {
            ...
            if (@tunnels == 0) {
                vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "tunnel"],
                    "$vpn_cfg_err No tunnels configured for peer \"$peer\".  At least" . " one tunnel required per peer.\n");
            }
        } else {
            #
            # Check if this is VTI
        ...
        foreach my $tunnel (@tunnels) {
            ...
            {
                vpn_die(
                    ["vpn", "ipsec", "site-to-site", "peer", $peer, $tunKeyword, "esp-group"],
                    "$vpn_cfg_err No ESP group specified for peer \"$peer\" " . "$tunKeyword.\n"
                );
            } elsif (!$vcVPN->exists("ipsec esp-group $peer_tunnel_esp_group")) {
                vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, $tunKeyword, "esp-group"],
                    "$vpn_cfg_err The ESP group \"$peer_tunnel_esp_group\" specified " . "for peer \"$peer\" $tunKeyword has not been configured.\n");
            }
                ...
                or $any_peer == 1)
            {
                if ($isVti) {
                    vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer],
                        "$vpn_cfg_err The peer \"$peer\" is invalid " . "ip address must be specified for $tunKeyword.\n");
                }
                $right    = '%any';
                $any_peer = 1;
            } else {
            ...
            if (!$isVti && ($leftsubnet_proto != $remotesubnet_proto)) {
                vpn_die(
                    ...
                    "$vpn_cfg_err The 'remote prefix' and 'local prefix' protocols " . "do not match"
                );
            }

            # Check remote/local and peer protocol consistency
            # XXX typo in below comment
            ...
            if (defined($lport)) {
                if (!defined($protocol)) {
                    ...
                    vpn_die(
                        ["vpn", "ipsec", "site-to-site", "peer", $peer, "tunnel", $tunnel, "local", "port"],
            ...
            if (defined($rport)) {
                if (!defined($protocol)) {
                    ...
                    vpn_die(
                        ["vpn", "ipsec", "site-to-site", "peer", $peer, "tunnel", $tunnel, "remote", "port"],
                    ...
                    if (defined($encryption) && defined($hash)) {
                        ...
                        if (defined($dh_group)) {
                            ...
                            if ($cipher_out eq 'unknown') {
                                vpn_die(["vpn","ipsec","site-to-site","peer",$peer,"tunnel", $tunnel],"$vpn_cfg_err Invalid 'dh-group' specified for ".
                ...
                foreach my $esp_proposal (@esp_proposals) {
                    ...
                    if (defined($pfs)) {
                        if ($pfs eq 'enable') {
                            ...
                            if ($pfs eq 'unknown') {
                                vpn_die(["vpn","ipsec","site-to-site","peer",$peer,"tunnel", $tunnel],"$vpn_cfg_err Invalid 'pfs dh-group' specified for ".
                                        "peer \"$peer\" $tunKeyword.");
                            }
                ...
                if ($espmode eq "transport") {
                    if (defined $leftsubnet or defined $rightsubnet) {
                        vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "$tunKeyword"],
                            "$vpn_cfg_err Can not use local-subnet or remote-subnet when " . "using transport mode\n");
                    }
                    if ($isVti == 1) {
                        vpn_die(
                            ["vpn", "ipsec", "site-to-site", "peer", $peer, "$tunKeyword"],
            ...
            if (!defined($auth_mode) || $auth_mode eq '') {
                ...
                if ($psk ne $orig_psk && $orig_psk ne "") {
                    ...
                    print "To force the key change use: 'clear vpn ipsec-peer'\n";
                }
                if (!defined($psk) || $psk eq '') {
                    vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "authentication"],
                        "$vpn_cfg_err No 'pre-shared-secret' specified for peer \"$peer\"" . " while 'pre-shared-secret' authentication mode is specified.\n");
                }
                ...
                if ($lip eq 'any' && !defined($dhcp_iface)) {
                    if ($index1 =~ m/^@/) {
                        ...
                        vpn_die(["vpn", "ipsec", "site-to-site", "peer", "0.0.0.0", "authentication"],
                            "$vpn_cfg_err cannot use authentication id with pre-shared-secret" . " when local-address is 0.0.0.0\n");
                    }
                    # when local-address is dynamic then only the following generic form works
                    $genout_secrets .= "%any $peer : PSK \"$psk\"\n";
                } else {
                ...
                unless (-r $local_key_file) {
                    if (-e $local_key_file) {
                        vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "authentication"],
                            "$vpn_cfg_err Invalid local RSA key file path " . "\"$local_key_file\".  Filesystem read permission absent.\n");
                    } else {
                        vpn_die(
                ...
                if (!defined($local_key) || $local_key eq "") {
                    vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "authentication"],
                        "$vpn_cfg_err Unable to determine local public key from local key" . " file \"$local_key_file\" for peer \"$peer\".\n");
                } else {
                ...
                if (!defined($rsa_key_name) || $rsa_key_name eq "") {
                    vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "authentication", "rsa-key-name"],
                        "$vpn_cfg_err No 'rsa-key-name' specified for peer \"$peer\"" . " while 'rsa' authentication mode is specified.\n");
                } else {
                    my $remote_key = $vcVPN->returnValue("rsa-keys rsa-key-name $rsa_key_name rsa-key");
                    if (!defined($remote_key) || $remote_key eq "") {
                        vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "authentication"],
                            "$vpn_cfg_err No remote key configured for rsa key name " . "\"$rsa_key_name\" that is specified for peer \"$peer\".\n");
                    } else {
                ...
                vpn_die(["vpn", "ipsec", "site-to-site", "peer", $peer, "authentication"],
                    "$vpn_cfg_err Unknown authentication mode \"$auth_mode\" for peer " . "\"$peer\" specified.\n");
            }
...
# but do not start the VPN daemon
        ...
        if (is_vpn_running()) {
            if (defined($update_interval) && ($update_interval != $update_interval_orig)) {
                ...
                vpn_exec(
                    'ipsec restart --auto-update ' . $update_interval . ' >&/dev/null',
...
sub dmvpnCheck {
    ...
    if (($enable && $vc->exists("vpn ipsec profile")) || ! $enable) {
        my @dmvpnOutput = `/opt/vyatta/sbin/vyatta-update-dmvpn.pl --ipsec`;
        my $exitCode = $?;
